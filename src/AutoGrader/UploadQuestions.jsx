...
const UploadQuestions = ({ setQuestionsAndMaxScores, handleNextStep, handlePreviousStep }) => {
    ...
    const handleManualSubmit = () => {
        if (!manualInput.trim()) {
            alert('Please enter the questions and maximum scores.');
            return;
        }
    
        const parsedQuestions = manualInput.split('\n')
            .filter(line => line.trim() !== '') // Remove any empty lines
            .map((line) => {
                return { question: line.trim(), maxScore: null }; // Leave maxScore as null if not provided
            });
    
        if (parsedQuestions.length === 0) {
            alert('No valid questions entered. Please provide at least one question.');
            return;
        }

        setQuestionsAndMaxScores(parsedQuestions);
        handleNextStep();
    };

    return (
        <div className="upload-questions-container">
            <h2>Step 1: Upload Questions with Maximum Scores</h2>
            <p>You can either upload a CSV file or manually enter the questions.</p>
